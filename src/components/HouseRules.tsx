...
import { houseRules } from '../constants/houseRules'
...
const HouseRules = ({ accepted, updateData }: HouseRulesProps) => {
  ...
  // Parse the house rules string into sections
  const sections = houseRules.split('\n\n').map((section, index) => {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-[#E67E22] mb-6">House Rules</h2>
      ...
      <div className="mt-6">
        <label className="flex items-center gap-2">
          ...
          <span className="text-sm text-[#2C3E50]">
            I have read and agree to comply with all house rules
          </span>
        </label>
      </div>
    </div>
